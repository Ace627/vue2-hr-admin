...
export default {
  ...
  methods: {
    async getApprovalsDetail() {
      const { data } = await getApprovalsDetail(this.approvalId)
      this.information = data
      this.information.data = JSON.parse(this.information.procData)
    },
    async getApprovalsTaskDetail() {
      const { data } = await getApprovalsTaskDetail(this.approvalId)
      this.taskInstanceOutList = data
    },
    // 图片下载
    async getReviewHistory(id) {
      const response = await downImg(id)
      this.imgs = 'data:image/png;base64,' + btoa(new Uint8Array(response.request.response).reduce((data, byte) => data + String.fromCharCode(byte), ''))
    },
  },
}
</script>
